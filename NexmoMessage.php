...
class NexmoMessage {
	...
	var $nx_voiceuri = 'https://rest.nexmo.com/tts/json';
	...
	function sendVoiceText ( $to, $message ,$lg='null' , $unicode='null') {
		...
		return $this->sendVoiceRequest ( $post );
	...
	private function sendVoiceRequest ( $data ) {
		// Build the post data
		$data = array_merge($data, array('username' => $this->nx_key, 'password' => $this->nx_secret));
		$post = '';
		foreach($data as $k => $v){
		...
		if (function_exists('curl_version')) {
			...
			$to_nexmo = curl_init( $this->nx_voiceuri );
			curl_setopt( $to_nexmo, CURLOPT_POST, true );
			curl_setopt( $to_nexmo, CURLOPT_RETURNTRANSFER, true );
			curl_setopt( $to_nexmo, CURLOPT_POSTFIELDS, $post );
			...
			$context = stream_context_create($opts);
			$from_nexmo = file_get_contents($this->nx_voiceuri, false, $context);
